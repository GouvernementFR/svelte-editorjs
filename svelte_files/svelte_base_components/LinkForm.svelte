<script>
    ...
    function saveCurrentSelection(selection) {
        ...
        savedSelection = selection.rangeCount === 0 ? null : selection.getRangeAt(0);
    ...
    export async function toggleLinkForm(e) {
        open = !open
        if (open) {
            const selection = document.getSelection();
            formInit(selection)
    ...
    async function createLink() {
        errorMsg = urlValidator(inputContent)
        ...
        if (inputIsValid) {
            ...
            if (targetBlank) {
                const link = document.getSelection().getRangeAt(0).startContainer.parentNode
                await tick()
                link.target = '_blank'
            }
            setCaretPosition()
...
<div class="linkform" class:open>
    ...
    <input class="fr-input" type="text" placeholder="https://" id="text-input-text" autocomplete="off"
           name="text-input-text"
           bind:value={inputContent} bind:this={input} on:blur={hideToolbar} on:keydown={hasEnterBeenPressed}>
    <div class="fr-checkbox-group fr-mt-1w fr-mb-2w" on:click|preventDefault={() => targetBlank = !targetBlank}>
        <input type="checkbox" id="checkbox-target" name="checkbox" bind:checked={targetBlank}>
        <label class="fr-label" for="checkbox-target">Ouvrir dans une nouvelle fenêtre</label>
    </div>
    <div class="fr-grid-row">
        {#if href}
            <button class="fr-btn fr-btn--secondary fr-col fr-mr-2w" type="button" on:click={deleteLink}>Supprimer
            </button>
        {/if}
        <button class="fr-btn fr-col" type="button" on:click={createLink}>Sauvegarder</button>
    </div>
